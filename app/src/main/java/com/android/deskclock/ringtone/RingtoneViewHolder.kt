...
internal class RingtoneViewHolder private constructor(itemView: View)
    ...
    override fun onBindItemView(itemHolder: RingtoneHolder) {
        ...
        if (itemViewType == VIEW_TYPE_CUSTOM_SOUND) {
            if (!itemHolder.hasPermissions()) {
                ...
                val colorPrimary = ThemeUtils.resolveColor(itemView.getContext(),
                        androidx.appcompat.R.attr.colorPrimary)
                mImageView.setColorFilter(colorPrimary, PorterDuff.Mode.SRC_ATOP)
            } else {
        ...
        itemView.setBackgroundColor(if (itemHolder.isSelected) {
            ThemeUtils.resolveColor(itemView.getContext(), com.google.android.material.R.attr.colorSurfaceContainerHigh)
        } else {
            Color.TRANSPARENT
        })
        if (itemViewType == VIEW_TYPE_CUSTOM_SOUND) {
    ...
    companion object {
        @JvmField val VIEW_TYPE_SYSTEM_SOUND: Int = R.layout.ringtone_item_sound
        @JvmField val VIEW_TYPE_CUSTOM_SOUND: Int = -R.layout.ringtone_item_sound
